...
Workflow: 
Downloading occurance data from iNaturalist using rinat package. Observations are filtered to ensure data quality is set to research grade, within the boundaries of the Western Cape. The filtered data is transformed into spatial objects using the sf package, with coordinate reference system conversion from WGS84 to UTM Zone 34S for accurate distance calculations. Visualisation of their distribution can be seen by both a static map and an interactive map, allowing individuals to explore observations and temporal patters in osytercatcher distribution.  
